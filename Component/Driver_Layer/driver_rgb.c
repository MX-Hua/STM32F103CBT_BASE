...
void RGB_SetnColor(uint8_t LedSrart, uint8_t LedNumber, RGB_Color_TypeDef Color)

{
    uint8_t i, j;

    if ((LedSrart + LedNumber) > LED_NUM) {
        return;    //avoid overflow 防止写入ID大于LED总数
    }

    for (j = LedSrart; j < LedSrart + LedNumber; j++) {
        for (i = 0; i < 8; i++) {
            Pixel_Buf[j][i]   = ((Color.G & (1 << (7 - i))) ? (CODE_1) : CODE_0);    //数组某一行0~7转化存放G
        }

        for (i = 8; i < 16; i++) {
            Pixel_Buf[j][i]   = ((Color.R & (1 << (15 - i))) ? (CODE_1) : CODE_0);    //数组某一行8~15转化存放R
        }

        for (i = 16; i < 24; i++) {
            Pixel_Buf[j][i]   = ((Color.B & (1 << (23 - i))) ? (CODE_1) : CODE_0);    //数组某一行16~23转化存放B
        }
    }
}
...
{
    HAL_TIM_PWM_Start_DMA(&WS2812_TIM, WS2812_TIM_CH, (uint32_t *)Pixel_Buf, (LED_NUM + 1) * 24);
...
void rgb_yellow_strip(uint16_t Pixel_Len)
...
void rgb_yellow_strip1(uint16_t Pixel_Len)
