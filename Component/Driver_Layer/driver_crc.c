...
uint8_t Driver_CRC8_Calculate(uint8_t *data, uint16_t length)
{
...
/**
@brief 查表法计算CRC8（速度快，需256字节RAM/Flash）
*/
...
// 预计算 CRC16-MODBUS 表（多项式 0xA001，256 项）
...
uint16_t Driver_CRC16_Modbus_Table_Fast(const uint8_t *data, uint32_t length)
{
    uint16_t crc = 0xFFFF;
    while (length--) {
        crc = (crc >> 8) ^ crc16_modbus_table[(crc & 0xFF) ^ *data++];
    }
    return crc; // 返回值：高字节=MSB, 低字节=LSB
}
